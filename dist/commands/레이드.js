import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, MessageFlags, SlashCommandBuilder, } from 'discord.js';
...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName('레이드').setDescription('레이드 선택').setRequired(true).setAutocomplete(true))
    .addStringOption((option) => option.setName('난이도').setDescription('난이도 선택').setRequired(true).setAutocomplete(true))
    .addStringOption((option) => option.setName('날짜').setDescription('예: 2026-08-01').setRequired(true).setAutocomplete(true))
    ...
    .setMaxValue(8)
    .setRequired(false))
    .addStringOption((option) => option.setName('메모').setDescription('참고 사항 (최대 500자)').setMaxLength(500).setRequired(false));
export const autocomplete = async (interaction) => {
    ...
    if (focused.name === '난이도') {
        ...
        const difficulties = Array.from(new Set(items
...
export const execute = async (interaction) => {
    ...
    try {
        ...
        await interaction.editReply('레이드 일정은 생성됐지만 상세 정보를 불러오지 못했습니다. 관리자에게 알려주세요.');
        return;
    }
    ...
    try {
        ...
        await interaction.editReply('레이드 일정은 등록됐지만 포럼 게시물 생성에 실패했습니다. 관리자에게 알려주세요.');
    }
};
//# sourceMappingURL=%EB%A0%88%EC%9D%B4%EB%93%9C.js.map