...
export const execute = async (interaction) => {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: '역할', value: roleLabels[user.role] ?? user.role, inline: true }, { name: '대표 캐릭터', value: user.mainCharacterName ?? '등록된 캐릭터 없음', inline: true });
    if (user.avatarUrl) {
        embed.setThumbnail(user.avatarUrl);
    }
    await interaction.editReply({ embeds: [embed] });
};
//# sourceMappingURL=%EB%82%B4%EC%A0%95%EB%B3%B4.js.map