...
function parseCharacterSelectCustomId(customId) {
...
export async function handleRaidApplyButton(interaction) {
    ...
    const select = new StringSelectMenuBuilder()
        ...
        .addOptions({ label: '딜러', value: 'dealer', emoji: '⚔️' }, { label: '서포터', value: 'support', emoji: '🛡️' });
    await interaction.editReply({
        content: '참여할 역할을 선택해주세요.',
        components: [new ActionRowBuilder().addComponents(select)],
    });
}
export async function handleRaidApplyRoleSelect(interaction) {
    ...
    const select = new StringSelectMenuBuilder()
        ...
        .addOptions(characters.slice(0, 25).map((character) => {
        const description = [
...
export async function handleRaidApplyCharacterSelect(interaction) {
...
//# sourceMappingURL=raid-apply.js.map