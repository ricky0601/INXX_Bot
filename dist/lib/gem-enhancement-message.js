...
function formatPremiumGahoEffect(effect) {
...
function parseAction(action) {
    ...
    if ('effect' in action.result)
        return { kind: 'gaho', effect: action.result.effect };
    return { kind: 'attempt', result: action.result };
}
function formatPercent(value) {
    return `${(value * 100).toFixed(1)}%`;
}
function formatPity(pity) {
    return `장기백 ${Math.round(pity)}%`;
}
function formatRates(rates) {
    return (`성공 ${formatPercent(rates.successProbability)} · ` +
...
function ratesAllZero(rates) {
    return (rates.successProbability === 0 &&
        rates.failureProbability === 0 &&
        rates.downProbability === 0 &&
        rates.destructionProbability === 0);
}
function formatGaho(state) {
    const parts = [];
    // Enhanced mode badge
...
function formatAttemptDetail(result) {
...
function buildStatusEmbed(view, parsed, footerName, botName) {
    ...
    let ratesToShow = state.currentRates;
...
function buildRankingEmbed(view) {
...
function buildButtons(view) {
    ...
    return new ActionRowBuilder().addComponents(new ButtonBuilder()
        ...
        .setDisabled(!canEnhance), new ButtonBuilder()
        .setCustomId(GEM_GAHO_DRAW_BUTTON_ID)
        .setLabel('가호 뽑기')
        .setStyle(ButtonStyle.Success)
        .setDisabled(!canUseGaho), new ButtonBuilder()
