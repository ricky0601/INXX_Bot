...
function formatParticipantHeadline(participant: RaidParticipant): string {
  return participant.userDisplayName ?? participant.userDiscordUsername ?? participant.characterName
}

function formatParticipantLine(participant: RaidParticipant): string {
...
export function buildRaidEmbed(detail: RaidScheduleDetail): EmbedBuilder {
  ...
  console.log(
    '[buildRaidEmbed] catalog.imageUrl=%s INXX_WEB_BASE_URL=%s resolved=%s',
    catalog.imageUrl,
    process.env.INXX_WEB_BASE_URL ?? '(unset)',
